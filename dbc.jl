...
# transition graph -- never assumed of the algorithm. For a candidate lattice
# symmetry g (a state-index permutation), if (i) the energy is g-invariant and
# (ii) the directed transition graph is g-equivariant -- every edge (s->d) maps to
# an edge (g.s -> g.d) carrying the IDENTICAL multiset of weight indices -- then for
# all couplings T(g.s->g.d) = T(s->d) and pi(g.s) = pi(s), so the detailed-balance
# residual of (g.s, g.d) is literally identical to that of (s,d). DB then need be
# evaluated on only one pair per orbit of the group generated by the verified g's.
#
# The candidates cover all elements of the full lattice point group p4m (all eight
# elements of D4 plus the two translation generators), so any subgroup -- D4, D2,
# C4, C2, a single reflection, translations only, any combination -- is detected.
# No assumption is made about WHICH subgroup is present; the graph decides.
#
# Soundness: equal weight-index multisets => identical interned Leaf objects =>
# identical symbolic weights (a SUFFICIENT, exact test); energy equality is exact
# integer-vector equality; a candidate g is admitted only if it is a genuine state
# bijection. A candidate that does not verify is simply dropped, so more pairs are
# checked, never fewer than correctness requires -- this can only speed the check,
# never alter the verdict. (Weight indices are canonical in serial runs; under
# -parallel, thread-local interning may split equal weights across indices, in
# which case a real symmetry fails to verify and is conservatively skipped.)
function _verified_graph_symmetry_reps(bfs::BFSResult,
        energy_coeffs::Vector{Vector{Q}}, pairs::Vector{Tuple{Int,Int}})
    states = bfs.states; idx = bfs.idx; n = bfs.n; S = length(states)
    ...
    # Build a candidate generator as a state-index permutation; nothing if it is
    # not a genuine bijection of the (closed) state set.
    function permof(f)
    ...
    # All 8 non-identity elements of the square-lattice point group D4, plus the
    # two translation generators, as individual candidates. Checking every element
    # (not just a generating set) means any subgroup -- D4, D2 (from C2 or two
    # reflections), C4, C2, a single reflection -- is detected correctly. Redundant
    # candidates (e.g. rotate180 is rotate90 twice) cost only one graph scan each
    # and the union-find merges their orbits automatically with no double-counting.
    cands = Tuple{String,Union{Nothing,Vector{Int}}}[
        ("translate(1,0)", permof(cs->translate_cstate(cs,1,0,n))),
        ("translate(0,1)", permof(cs->translate_cstate(cs,0,1,n))),
        ("rotate90",       permof(cs->rotate_cstate(cs,n))),
        ("rotate180",      permof(cs->rotate180_cstate(cs,n))),
        ("rotate270",      permof(cs->rotate270_cstate(cs,n))),
        ("reflect",        permof(cs->reflect_cstate(cs,n))),
        ("reflect_h",      permof(cs->reflect_h_cstate(cs,n))),
        ("reflect_v",      permof(cs->reflect_v_cstate(cs,n))),
        ("reflect_ad",     permof(cs->reflect_ad_cstate(cs,n)))]
    verified = Vector{Int}[]; names = String[]
    for (nm,π) in cands
        π === nothing && continue
        all(energy_coeffs[π[i]] == energy_coeffs[i] for i in 1:S) || continue   # (i) energy
        equ = true                                                              # (ii) graph
        for ((s,d),w) in ew
            w2 = get(ew, (π[s],π[d]), nothing)
            (w2 === nothing || w2 != w) && (equ = false; break)
        end
        equ && (push!(verified, π); push!(names, nm))
    end
