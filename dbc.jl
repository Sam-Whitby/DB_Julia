...
# Square-lattice point-group (D4) actions on a concrete state: a 90-degree
# rotation and a diagonal reflection (these two generate all of D4). Both are
# lattice isometries, so they preserve squared minimum-image distances. They are
# used ONLY to reduce the detailed-balance pair check, and only after the action
# is VERIFIED to be a symmetry of the already-computed transition graph and energy
# (see SECTION 7) -- nothing about the algorithm is assumed.
