...
function parse_args(argv)
    ...
    maxdepth = 30; i = 2
    while i <= length(argv)
        argv[i] == "-maxdepth" ? (maxdepth = parse(Int, argv[i+1]); i += 2) :
...
function run_checker(algfile, maxdepth, n, types, algo, energy)
    ...
    try
        ...
        e isa CantHandle ? (println("  ERROR: ", e.msg); exit(1)) : rethrow(e)
    end
    ...
    try
        ...
        td = @elapsed ((pass, viol, nch) = run_db_check(m))
...
run_checker(_ALGFILE, _MAXDEPTH, Main.NGRID, Main.PARTICLE_TYPES, Main.algorithm, Main.energy)
